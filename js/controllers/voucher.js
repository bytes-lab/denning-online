denningOnline
  .controller('voucherListCtrl', function(NgTableParams, voucherService, Auth, $state) {
    var self = this;
                                          ...
                                          uibDateParser, $uibModal, NgTableParams) 
  {
    ...
    if ($stateParams.id) {
      self.title = 'Edit Voucher';
      voucherService.getItem($stateParams.id).then(function (item){
    ...
    self.save = function () {
      ...
      voucherService.save(entity, self.entity_).then(function (item) {
