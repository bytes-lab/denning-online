denningOnline
    .controller('CKHTListCtrl', function($filter, $sce, $uibModal, NgTableParams, CKHTService) {
        ...
        CKHTService.getList().then(function(data) {
            self.data = data;
            self.dataReady = true;
            initializeTable();
        });        
        ...
        function modalInstances(animation, size, backdrop, keyboard, contact) {
            var modalInstance = $uibModal.open({
                ...
                controller: 'CKHTDeleteModalCtrl',
                size: size,
                backdrop: backdrop,
                keyboard: keyboard,
                resolve: {
    ...
    .controller('CKHTDeleteModalCtrl', function ($scope, $uibModalInstance, contact, CKHTService, $state) {
        $scope.ok = function () {
            CKHTService.delete(contact).then(function(contact) {
                $state.reload();
            })
            .catch(function(err){
                //Handler
    ...
    .controller('CKHTEditCtrl', function($filter, $stateParams, CKHTService, $state) {
        var self = this;
        self.save = save;

        if($stateParams.id) {
            CKHTService.getItem($stateParams.id)
            .then(function(item){
                self.contact = item;
            });
        } else {
        ...
        function save() {
            CKHTService.save(self.contact).then(function(contact) {
