denningOnline
  ...
  .controller('landOfficeEditCtrl', function($stateParams, landOfficeService, $state, $uibModalInstance, 
                                             ...
                                             cityService, Auth) 
  {
    ...
    self.fullAddress = function () {
      fullAddress = '';
      if (self.entity) {
        if (self.entity.strLandadministrator)
          fullAddress = self.entity.strLandadministrator.trim().toUpperCase()+'\n';
    ...
    if (self.entityCode) {
      self.title = 'Edit Land Office';

      landOfficeService.getItem(self.entityCode).then(function (item) {
        ...
        self.popoutUrl = $state.href('land-offices.edit', { id: self.entity.code });
      });
    } else {
      self.title = 'New Land Office';

      self.entity = {};
      
      self.popoutUrl = $state.href('land-offices.new');
    }
        ...
        if (contact) {  // ignore when errors
          if (self.isDialog) {
            ...
            if (self.entity_) {
              ...
              $state.go('land-offices.edit', { 'id': contact.code });
            }
